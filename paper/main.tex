...
Schedule comparisons show that non-constant schedules (including the PMP-derived one) dramatically outperform constant baselines, with adaptive decay schedules reducing regret by up to $90\%$.
\end{abstract}
...
\citet{starlhf2024} introduced Self-Play Preference Optimization (SPO), framing preference learning as a zero-sum game and proving that a single agent playing against itself can find the minimax winner without a reward model.
