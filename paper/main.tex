...
\begin{enumerate}[leftmargin=1.5em,itemsep=1pt,topsep=2pt]
  ...
  \item \textbf{Principled KL schedule} (Proposition~\ref{thm:schedule}): Applying PMP to the outer optimization over $\beta(\cdot)$, we motivate a non-constant schedule $\beta^*(t)$ that starts above $\beta_c$, decays toward it, and includes an oscillatory correction term matched to the system's natural frequency. We show that all non-constant schedules dramatically outperform constant baselines.
\end{enumerate}
...
Let $\mu_s > 0$ denote the spectral gap of $\bA_{\mathrm{cl}}(\beta_c)$ restricted to the stable manifold, and let $\omega_c$ be the Hopf frequency from Theorem~\ref{thm:hopf}.
...
Proposition~\ref{thm:schedule} provides a principled \emph{functional form} for KL annealing.
...
The LQ game (Theorems~\ref{thm:riccati}--\ref{thm:hopf}) determines the bifurcation threshold $\beta_c$ and crossing frequency $\omega_c$ from the linearized dynamics near the Nash equilibrium---these are \emph{local} properties that do not depend on the global nonlinear structure.
However, the LQ model diverges for $\beta < \beta_c$; producing stable limit cycles requires the cubic saturation terms present in the full nonlinear model.
Thus the two models play complementary roles: the LQ analysis identifies \emph{when} and \emph{how fast} cycling begins, while the Hopf normal form (with nonlinear saturation) predicts the \emph{amplitude} of the resulting limit cycle.
The phase portraits and bifurcation diagrams below use the nonlinear model, while the Riccati-based schedule analysis uses the LQ model.
...
PMP (Prop.~\ref{thm:schedule}) & 1.526 & 0.0000 \\
\bottomrule
\end{tabular}
